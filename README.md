...
The compatibility table rejects unknown behavioral fields and changes to unmodeled scalar settings. Known cosmetic fields are ignored. This intentionally limits imports to the supported family; it is not a general parser that silently turns other drills into Pasu. Missing external profiles, projectiles, weapon chains, damage-based scores, finite lives and other unsupported essential behavior must be rejected.
